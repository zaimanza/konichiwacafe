						...
						<c:forEach items="${order}" var="orders">
							...
							<tr>
								...
								<c:if test="${orders.getOrderStatus().equals('2')}">
									<td><c:out value="Preparing" /></td>
								</c:if>  
								<c:if test="${orders.getOrderStatus().equals('3')}">
									<td><c:out value="Ready to deliver" /></td>
								</c:if>
								<c:if test="${orders.getOrderStatus().equals('4')}">
									<td><c:out value="Picked up by Rider" /></td>
								</c:if>
								<c:if test="${orders.getOrderStatus().equals('5')}">
									<td><c:out value="Completed" /></td>
								</c:if>
								<td><a href="OrdersController?action=orderdetails&i=<c:out value="${orders.orderID}" />&RiderId=<c:out value="${orders.riderID}" />">
