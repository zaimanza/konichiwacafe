		...
		<c:if test="${not empty addMsg}">
          <div id="close" class="alert success">
 			<span class="closebtn" onclick="document.getElementById('close').style.display='none'">&#215;</span>
          	<h6 style="color:white"><c:out value="${addMsg}"></c:out></h6>
          </div>
        </c:if>
        <% request.removeAttribute("addMsg");%>
			...
			<div class="cart-table">
				<form name="checkoutForm" method="post" action="OrdersController?action=checkout" onsubmit="successPymt()">
					<table class="address">
						<tr>
							...
							<c:choose>
								...
								<c:otherwise>
									<i><c:out value="No address added"></c:out></i>
								</c:otherwise>
							</c:choose>
							</td>
						</tr>
					...
					<a	href="AddressController?action=addAddressCart&i=<%=session.getAttribute("custid")%>" class="primary-btn" >Add New Address</a>
					&nbsp;&nbsp;&nbsp;&nbsp;
