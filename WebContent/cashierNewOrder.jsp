...
<title>New Order | Konichiwa Cafe</title>
<style>
	.center 
	{
	   	display: block;
	...
	<div class="center">
		...
		<div id="Pickup" >
			<div class="cart-table">		
	  			<c:set var="orderCheck" value="${listNew}"/>
					<c:choose> 
						<c:when test="${orderCheck != null}">
							<table class="address">	
								<tr>
					        		...
					        		<c:forEach items="${listNew}" var="order">
						        		<tr>
						            		<td><c:out value="${order.orderID}" /></td>        
						                    <td><c:out value="${order.orderDate}" /></td>  
						                    <td><c:out value="${order.getDeliveryAddress()}" /></td>  
						                    <td><c:out value="${order.custID}" /></td>   
						                    <td><a class="orderid" href="OrdersController?action=cashierNewOrderdetails&i=<c:out value="${order.orderID}" />&custId=<c:out value="${order.custID}" />&RiderId=${order.riderID}">
						                    	<button class="updatebtn" style="border:none">View</button>
						                    </a></td> 
						                   	<td><a href="RiderController?action=cashierViewAvailableRiders&orderid=${order.orderID}"><button class="removebtn" style="border:none">Assign</button></a></td>    
						          		</tr>
					      			</c:forEach>
							</table>	
						</c:when>
