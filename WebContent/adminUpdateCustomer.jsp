...
<title>Customer Details | Konichiwa Cafe</title>
</head>
<body class="example">
			...
			<div class="row">
				<div class="col-lg-6 offset-lg-3">
					<div class="register-form">
						...
						<c:if test="${customer.availability.equals('1')}">
							<c:set var="avail" value="Disable"></c:set>
						</c:if>
						<c:if test="${customer.availability.equals('2')}">
							<c:set var="avail" value="Enable"></c:set>
						</c:if>
						
						<form name="update" action="CustomerController?action=updateAvailability"
							method="post">
													
							<input type="hidden" name="avail" value="<c:out value='${customer.availability}'/>">
							<c:if test="${avail.equals('Disable')}">
							<button type="submit" name="custid" class="btndisable primary-btn"
								value="<c:out value='${customer.custid}' />"><c:out value="${avail}"/></button>
							</c:if>
							<c:if test="${avail.equals('Enable')}">
							<button type="submit" name="custid" class="btnenable primary-btn"
								value="<c:out value='${customer.custid}' />"><c:out value="${avail}"/></button>
							</c:if>
						</form>
						<a href="#" onclick="viewCustomer()"><button class="primary-btn" style="border:none; float:right;">Back</button></a>
						</div>
