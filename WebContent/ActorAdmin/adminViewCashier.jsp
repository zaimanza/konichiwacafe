...
<head>
		...
		<meta charset="ISO-8859-1">
		<title>Cashier| Konichiwa Cafe</title>
	</head>
<body class="example" class="center">
	...
	<table class="list">
		...
		<c:forEach items="${cashierlist}" var="cash">
        		<%bill++; %>
        		<tr>
					<td><% out.print(bill); %></td>  
					<td><c:out value="${cash.getStaffFName()} ${cash.getStaffLName()}"/></td>
					<td><c:out value="${cash.getStaffId()}"/></td>
					<td><c:out value="${cash.getStaffPhone()}"/></td>
					<td><c:out value="${cash.getStaffEmail()}"/></td>
					<c:if test="${ cash.getStaffType() == 0 }">
						<td><a href="StaffController?action=adminVerifyCashier&id=<c:out value='${cash.getStaffId()}'/>" style="color:green;">Verify</a></td>
					</c:if>
					<c:if test="${ cash.getStaffType() == 1 }">
						<td><a href="StaffController?action=adminUnVerifyCashier&id=<c:out value='${cash.getStaffId()}'/>" style="color:red;">Invalidate</a></td>
					</c:if>
					<td><a href="StaffController?action=adminUpdateCashier&id=<c:out value='${cash.getStaffId()}'/>"><button class="updatebtn" style="border:none;">Update</button></a></td>
					<td><a href="StaffController?action=adminDeleteCashier&id=<c:out value='${cash.getStaffId()}'/>"><button class="deletebtn" style="border:none">Delete</button></a></td>
