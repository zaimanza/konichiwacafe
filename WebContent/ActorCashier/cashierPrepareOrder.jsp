							...
							<table class="address">	
								<tr>
					        		...
					        		<c:forEach items="${listPrepare}" var="order">
						        		<tr>
						            		<td><a class="orderid" href="OrdersController?action=cashierPrepareOrderdetails&i=<c:out value="${order.orderID}" />"><c:out value="${order.orderID}" /></a></td>        
						                    <td><c:out value="${order.orderDate}" /></td>  
						                    <td><c:out value="${order.getDeliveryAddress()}" /></td>  
						                    <td><c:out value="${order.custID}" /></td>  
						                   	<td><c:out value="${order.staffID}" /></td>  
						                   	<td><a href="OrdersController?action=cashierOrderPrepare&orderid=${order.orderID}"><button class="removebtn" style="border:none">Done</button></a></td>    
						          		</tr>
					      			</c:forEach>
							</table>	
						</c:when>
