...
<title>Report | Konichiwa Cafe</title>
</head>
<style>
	.center 
	{
...
<script type="text/javascript">
	...
	<div class="cart-table">
		...
		<table>
			<tr>
				<th style="background-color:#e7ab3c;">Total Sales</th>
			</tr>
			
			<tr>
				<th style="color:green;">RM <fmt:formatNumber type="number" maxFractionDigits="2" minFractionDigits="2" value="${allTotalPrice}"/></th>
			</tr>
		</table>
		...
		<c:set var="orderCheck" value="${ordersHistory}"/>
		<c:choose> 
					...
					<tr>
		        		...
		        		<c:forEach items="${ordersHistory}" var="order">
			        		<tr>
			                    ...
			                    <td><c:out value="${order.getCustID()}" /></td>
			                    <td><c:out value="${order.getRiderID()}" /></td>  
			                    <td><c:out value="${order.getStaffID()}" /></td> 
			                   	<td><a class="orderid" href="OrdersController?action=adminViewOrderDetails&i=<c:out value="${order.orderID}" />&custId=<c:out value="${order.custID}" />&RiderId=${order.riderID}">
			                   		<button class="updatebtn" style="border:none">View</button>
			                   	</a></td>        
			          		</tr>
		      			</c:forEach>
				</table>	
